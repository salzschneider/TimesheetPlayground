...
namespace TimesheetPlayground.UI.BLL
{
    public class TimesheetService : ServiceBase, ITimesheetService
    {
        ...
        public async Task<TimesheetDO> InsertTimesheetAsync(TimesheetDO timesheetDO)
        {
            ...
            {
                if (ue.InnerException is SqlException sqlEx)
                {
        ...
        private static void ValidateTimesheet(Timesheet timesheet, TimesheetDO timesheetDO)
        {
            if (timesheet == null)
            {
        ...
        public async Task ChangeTimesheetStatusAsync(int timesheetId, StatusEnum status, bool IsManagerUser)
        {
            ...
            var timesheet = await dbContext.Timesheets.FirstOrDefaultAsync(t => t.Id == timesheetId) ?? throw new InvalidOperationException("Timesheet does not exist");
